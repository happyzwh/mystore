...
$.extend($.validator, {
	...
	messages: {
		...
		equalTo: "请再次输入相同的值",
		maxlength: $.validator.format("最多输入 {0} 字节(一个中文字算2个字节)"),
		minlength: $.validator.format("最少输入 {0} 字节(一个中文字算2个字节)"),
		...
		showLabel: function( element, message ) {
			...
			if ( label.length ) {
				...
				if ( label.attr("generated") ) {
					label.html(message+'<span class=val_corner></span><span class="errorIcon"></span>');
				}
			} else {
				// create label
				label = $("<" + this.settings.errorElement + "/>")
					...
					.html(message+'<span class=val_corner></span><span class="errorIcon"></span>' || "");
				if ( this.settings.wrapper ) {
				...
				if ( !this.labelContainer.append(label).length ) {
					if ( this.settings.errorPlacement ) {
                        ...
                        label.css({display:"block",position:"absolute",left:$(element).offset().left+$(element).outerWidth()-20,top:$(element).offset().top-32}).insertAfter(element);
					}
				}
			}
